use crate::lcu::types::{MatchPerformance, PlayerMatchStats, SummonerTrait};
...
fn analyze_win_rate_traits(stats: &PlayerMatchStats, traits: &mut Vec<SummonerTrait>) {
    if stats.win_rate >= 65.0 {
        traits.push(SummonerTrait {
            name: "大神".to_string(),
            description: "胜率超高的实力玩家".to_string(),
        ...
        traits.push(SummonerTrait {
            name: "稳定".to_string(),
            description: "胜率稳定的可靠队友".to_string(),
            score: stats.win_rate as i32,
            trait_type: "good".to_string(),
        });
    } else if stats.win_rate <= 40.0 && stats.total_games >= 10 {
        traits.push(SummonerTrait {
            name: "坑货".to_string(),
            description: "胜率偏低的玩家".to_string(),
            score: stats.win_rate as i32,
...
fn analyze_kda_traits(stats: &PlayerMatchStats, traits: &mut Vec<SummonerTrait>) {
    if stats.avg_kda >= 4.0 {
        traits.push(SummonerTrait {
            name: "大爹".to_string(),
            description: "KDA超高的carry玩家".to_string(),
            score: (stats.avg_kda * 10.0) as i32,
            trait_type: "good".to_string(),
        });
    } else if stats.avg_deaths > stats.avg_kills * 1.8 && stats.avg_deaths >= 7.0 {
        traits.push(SummonerTrait {
            name: "送分".to_string(),
            description: "死亡次数过多的玩家".to_string(),
...
fn analyze_streak_traits(stats: &PlayerMatchStats, traits: &mut Vec<SummonerTrait>) {
    ...
    if win_streak >= 5 {
        traits.push(SummonerTrait {
            name: "连胜王".to_string(),
            description: "近期状态火热的玩家".to_string(),
            score: win_streak,
            trait_type: "good".to_string(),
        });
        ...
        traits.push(SummonerTrait {
            name: "连败".to_string(),
            description: "近期状态不佳的玩家".to_string(),
