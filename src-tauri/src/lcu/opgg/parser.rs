...
pub fn parse_champion_build(data: Value, position: &str) -> Result<OpggChampionBuild, String> {
    ...
    let name = summary_obj.get("name").and_then(|v| v.as_str()).unwrap_or("").to_string();
...
fn parse_summoner_spells(content: &Value) -> Result<Vec<OpggSummonerSpell>, String> {
    let spells_array = content.get("summoner_spells").and_then(|v| v.as_array()).ok_or("无法获取召唤师技能数据")?;
    let mut spells = Vec::new();
    for spell_data in spells_array.iter() {
        let ids: Vec<i32> = spell_data.get("ids").and_then(|v| v.as_array()).map(|arr| arr.iter().filter_map(|v| v.as_i64().map(|i| i as i32)).collect()).unwrap_or_default();
...
fn parse_item_category(content: &Value, category: &str) -> Result<Vec<OpggItem>, String> {
    ...
    for item_data in category_data.iter() {
        let ids: Vec<i32> = item_data.get("ids").and_then(|v| v.as_array()).map(|arr| arr.iter().filter_map(|v| v.as_i64().map(|i| i as i32)).collect()).unwrap_or_default();
...
fn parse_counters(content: &Value) -> Result<OpggCounters, String> {
    ...
    Ok(OpggCounters { strong_against, weak_against })
}
...
fn parse_perks(content: &Value) -> Result<Vec<OpggPerk>, String> {
    ...
    for rune in runes_array.iter() {
        ...
        let primary_rune_ids: Vec<i32> = rune.get("primary_rune_ids").and_then(|v| v.as_array()).map(|arr| arr.iter().filter_map(|v| v.as_i64().map(|i| i as i32)).collect()).unwrap_or_default();
        let secondary_rune_ids: Vec<i32> = rune.get("secondary_rune_ids").and_then(|v| v.as_array()).map(|arr| arr.iter().filter_map(|v| v.as_i64().map(|i| i as i32)).collect()).unwrap_or_default();
        let stat_mod_ids: Vec<i32> = rune.get("stat_mod_ids").and_then(|v| v.as_array()).map(|arr| arr.iter().filter_map(|v| v.as_i64().map(|i| i as i32)).collect()).unwrap_or_default();
        // 按主系4+副系2+属性3顺序拼接
...
fn parse_champion_skills(content: &Value) -> Result<OpggSkills, String> {
    ...
    if let Some(skill_masteries) = content.get("skill_masteries").and_then(|v| v.as_array()) {
        if let Some(main) = skill_masteries.get(0) {
            let masteries = main.get("ids").and_then(|v| v.as_array())
            ...
            let order = main.get("builds")
                ...
                .map(|arr| arr.iter().filter_map(|v| v.as_str().map(|s| s.to_string())).collect())
                .unwrap_or_default();
            return Ok(OpggSkills { masteries, order, play, win, pick_rate });
        }
    }
    // fallback: 兼容旧数据
    let binding = vec![];
    ...
    if let Some(skill) = skills_array.get(0) {
        order = skill.get("order").and_then(|v| v.as_array()).map(|arr| arr.iter().filter_map(|v| v.as_str().map(|s| s.to_string())).collect()).unwrap_or_default();
