...
- [🎮 Nidalee](#-nidalee)
  ...
  - [🤝 贡献](#-贡献)
    ...
    - [发布流程](#发布流程)
  - [🔄 自动更新（Tauri Updater）](#-自动更新tauri-updater)
  - [📦 分发与签名政策](#-分发与签名政策)
  - [🌐 网络与下载说明](#-网络与下载说明)
  - [🛠️ 故障排查](#️-故障排查)
  - [📄 许可证](#-许可证)
...
本项目已集成 Tauri v2 Updater 插件，实现自动检查、下载、安装并重启。

- 后端：已注册 tauri-plugin-updater 与 tauri-plugin-process（`src-tauri/src/lib.rs`）
- 前端：已安装 @tauri-apps/plugin-updater 与 @tauri-apps/plugin-process，并在启动后静默检查更新
- 权限：`src-tauri/capabilities/default.json` 已授予 `updater:default` 与 `process:default`

使用前需要完成以下配置：

1. 生成并配置签名密钥

   - 使用 Tauri CLI 生成密钥对；将“公钥内容”粘贴到 `src-tauri/tauri.conf.json` → `plugins.updater.pubkey`
   - 在构建环境中设置私钥环境变量（本地或 CI）：
     - `TAURI_SIGNING_PRIVATE_KEY`（必需）
     - `TAURI_SIGNING_PRIVATE_KEY_PASSWORD`（如果创建时设置了密码）
   - 本地构建可在 shell 临时导出，或在 CI 中配置为仓库 Secrets。

2. 配置更新源 latest.json

   - 使用 GitHub Releases 静态端点：[https://github.com/codeXcn/Nidalee/releases/latest/download/latest.json](https://github.com/codeXcn/Nidalee/releases/latest/download/latest.json)
   - 确保 CI 发布时上传 `latest.json` 与安装包（MSI/DMG），且签名与公钥匹配

3. 构建产物

   - 已启用 `bundle.createUpdaterArtifacts: true` 以生成 Updater 所需文件
   - Windows 安装模式默认 `passive`，可在 `tauri.conf.json` 调整

4. 运行时行为

   - 启动后静默检查更新；有新版本时侧边栏会出现“有可用更新”，点击后直接下载并安装，完成后自动重启
   - 下载过程中会显示进度条与动态文案：
     - 0%：正在连接更新服务器…
     - 1–99%：正在下载更新 N%
     - 100%：正在安装/准备重启…
   - 若更新失败，应用会弹出带「前往手动下载」的提示，跳转到官方 Releases 最新页

5. CI（GitHub Actions）

   - 工作流位于 `.github/workflows/release.yml`，推送以 `v*` 开头的 tag 时触发
   - 需要在仓库添加 Secrets：
     - `TAURI_SIGNING_PRIVATE_KEY`
     - `TAURI_SIGNING_PRIVATE_KEY_PASSWORD`（如果有）

更多参考：[Tauri Updater 文档](https://v2.tauri.app/zh-cn/plugin/updater/)

## 📦 分发与签名政策

- 本项目仅在本仓库的 [Releases](https://github.com/codeXcn/Nidalee/releases) 页面发布官方安装包与更新文件。
- Updater 仅信任 `src-tauri/tauri.conf.json` 中配置的官方签名公钥，第三方构建无法通过应用内自动更新。
- 禁止在非本仓库发布发行版或重新分发本项目的打包产物；如需参与改进，请提 Issue/PR，一起在本仓库发布。
